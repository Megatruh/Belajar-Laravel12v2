<nav id="navbar" class="bg-gray-800 fixed top-0 left-0 right-0 z-50 transition-transform duration-300 ease-in-out">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
    <div class="flex h-16 items-center justify-between">
        <div class="flex items-center">
            <div class="shrink-0">
                <img src="{{ asset('IMG/newLogo.png') }}" alt="Your Company" class="size-10" />
            </div>
            <div class="hidden md:block">
                <div class="ml-10 flex items-baseline space-x-4">
                <!-- Current: "bg-gray-900 text-white", Default: "text-gray-300 hover:bg-white/5 hover:text-white" -->
                <x-my-nav-link href="/" :current="request()->is('/')">Home</x-my-nav-link>
                <x-my-nav-link href="/blog" :current="request()->is('blog')">Blog</x-my-nav-link>
        ...
        <div class="hidden md:block">
            <div class="ml-3 flex items-center md:ml-6">
                ...
                <button class="relative flex max-w-xs items-center rounded-full focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500 cursor-pointer">
                    ...
                    <img src="{{ asset('IMG/Profile/farhan.jpg') }}" alt="" class="size-8 rounded-full outline -outline-offset-1 outline-white/10" />
                    <div class="ml-3 text-gray-300 font-medium">{{ Str::limit(Auth::user()->name, 20)}}</div>
                </button>
                ...
                <el-menu anchor="bottom end" popover class="w-48 origin-top-right rounded-md bg-white py-1 shadow-lg outline-1 outline-black/5 transition transition-discrete [--anchor-gap:--spacing(2)] data-closed:scale-95 data-closed:transform data-closed:opacity-0 data-enter:duration-100 data-enter:ease-out data-leave:duration-75 data-leave:ease-in">
                    ...
                    <form action="/logout" method="POST">
                        @csrf
                        <button type="submit" class="w-full text-start block px-4 py-2 text-sm text-gray-700 focus:bg-gray-100 focus:outline-hidden cursor-pointer">Log out</button>
                    </form>
                </el-menu>
                </el-dropdown>
                @else
    ...
    <div class="space-y-1 px-2 pt-2 pb-3 sm:px-3">
        ...
        <x-my-nav-link class="block" href="/blog" :current="request()->is('blog')">Blog</x-my-nav-link>
    ...
    <div class="border-t border-white/10 pt-4 pb-3">
        @if(Auth::check())
            ...
            <div class="mt-3 space-y-1 px-2">
                ...
                <form action="/logout" method="post">
                    @csrf
                    <button type="submit" class="w-full text-start block rounded-md px-3 py-2 text-base font-medium text-gray-400 hover:bg-white/5 hover:text-white">Log out</button>
                </form>
            </div>
        @else
            <a href="/login" class="block rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white">Login</a>
            <a href="/register" class="block rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white">Register</a>
        @endif
